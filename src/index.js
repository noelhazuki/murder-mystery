...
if (url.searchParams.get("ask") === "true") {
  ...
  // categoryが付いてる場合（テンプレート経由）は、質問文をClaudeに渡す前に
  // 「{major}カテゴリの{sub}の相談：{freeText}」の形に結合する（2026-07-22確定）。
  // categoryが無い場合（自由入力）はquestionをそのまま渡す。
  const combinedQuestion =
    category && category.major && category.sub
  ...
  try {
    ...
    // newConcernsはAIからは文字列配列で返ってくるので、ここでid付与・category付与を行う。
    // categoryは今回のリクエストのものをそのままコピーする（AIによる個別判定はしない、2026-07-22確定）。
    const newConcernObjects = claudeResult.newConcerns.map((text) => ({
      id: crypto.randomUUID(),
      text,
      category: category || null
    }));

    const updatedConcerns = [...openConcerns, ...newConcernObjects];
    await env.KV.put(key, JSON.stringify({ ...deck, openConcerns: updatedConcerns }));

    return new Response(
      JSON.stringify({ ok: true, answer: claudeResult.answer, newConcerns: newConcernObjects }),
      { headers: { "Content-Type": "application/json", ...CORS_HEADERS } }
    );
  } catch (e) {
    return new Response(
...
// ▼ 未解決懸念点の解決処理 (resolve_concerns)
// openConcerns（{id, text, category}の配列）から、渡されたidに一致するものを削除する。
// 複数id同時指定に対応（画面側でチェックボックス複数選択→まとめて1回のリクエスト、2026-07-23確定）。
// 見つかったidはその場で削除、見つからんかったidはnotFoundIdsに入れて返す（処理自体は止めない）。
// idsが空配列の場合はバリデーションエラーとして弾く（2026-07-23確定）。
if (url.searchParams.get("resolve_concerns") === "true") {
  ...
  if (!deckId || !Array.isArray(ids)) {
    return new Response(
      JSON.stringify({ ok: false, error: "deckId/idsは両方必須やで（idsは配列で）" }),
      { status: 400, headers: { "Content-Type": "application/json", ...CORS_HEADERS } }
    );
  }
  ...
  const deletedIds = ids.filter((id) => openConcerns.some((c) => c.id === id));
  const notFoundIds = ids.filter((id) => !openConcerns.some((c) => c.id === id));

  if (deletedIds.length > 0) {
    const updatedConcerns = openConcerns.filter((c) => !deletedIds.includes(c.id));
    await env.KV.put(key, JSON.stringify({ ...deck, openConcerns: updatedConcerns }));
  }

  return new Response(
    JSON.stringify({ ok: true, deletedIds, notFoundIds }),
    { headers: { "Content-Type": "application/json", ...CORS_HEADERS } }
  );
}
// ▲ 未解決懸念点の解決処理 (resolve_concerns)
